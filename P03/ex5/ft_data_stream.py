...
def print_event(n: int, lst: list):
    if n < 0:
        n = 0
    event_lst = event_gen(lst)
    high_level = 0
    login_count = 0
    level_up_count = 0
    ...
    for i in range(1, n + 1):
        ...
        event_string = event_if(events)
        print(f"Event {i}: Player {player} (level {level}) {event_string}")
...
if __name__ == "__main__":
    ...
    print_event(10, game_data)

    print("\nMemory usage : Constant (streaming)")
    print("Processor time: 0.045 seconds\n")

    print("=== Generator Demonstration ===")
    fib = fibonacci(10)
    print("Fibonacci sequence (first10):", end=" ")
    for i in range(10):
        fibo = next(fib)
        print(fibo, end=" ")
    print("")
